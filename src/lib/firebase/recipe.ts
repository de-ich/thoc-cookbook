...
export const getRecipeDetails = async (recipeId: string, fromServer: boolean = false) => {
	...
	const recipeDocument = await recipeDocumentPromise.then(
		(document) => document.data() as RecipeDetails
	);
...
export const getAllKeywords = async () => {
	...
	return getDoc(keywordsRef)
		.then((document) => document.data() as Map<string, any>)
		.then((d) => Object.keys(d));
};

export const checkRecipeWithSourceIdDoesNotYetExist = async (
	sourceId: string | number
): Promise<boolean> => {
	const q = query(recipeDetailsCollectionRef, where('sourceId', '==', sourceId));
	return await getDocs(q)
		.then((snapshot) => snapshot.docs)
		.then((documents) => documents && documents.length > 0);
};
...
export const addRecipe = async (recipeDraft: RecipeDraft | RecipeDetails): Promise<string> => {
	...
	const existingOrEmptyDocoument = await getRecipeDetails(docId, true)
		.then((recipe) => recipe)
		.catch(() => getEmptyRecipeDraft());
...
const uploadLocalImageToStorage = async (
	image: File,
...
const copyRemoteImageToStorage = async (
	imageUrl: string,
	folderRef: StorageReference
): Promise<string> => {
	const downloadedFile = await downloadFileCallable({ url: imageUrl })
		.then((result) => result.data as DownloadedFile)
		.catch((error) => {
			throw Error(
				`The following error occurred while trying to download image from URL ${imageUrl}: ${error.message || ''}`
			);
		});
