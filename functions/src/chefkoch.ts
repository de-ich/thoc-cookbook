...
const internalFetchRecipe = async (recipeId: string, chefkochCookie?: string) => {
	...
	if (!response.ok) {
		throw new HttpsError(
			'not-found',
			`Unable to retrieve recipe with ID ${recipeId} from chefkoch.de! The following error occurred: ${response.statusText}`
		);
	}
...
const convertToPartialRecipe = (chefkochRecipe: any): RecipeDraft => {
	...
	for (const ingredientGroup of chefkochRecipe.ingredientGroups || []) {
		for (const chefkochIngredient of ingredientGroup.ingredients) {
			...
			try {
				ingredientString =
					[
						chefkochIngredient.amount || '',
						chefkochIngredient.unit || '',
						chefkochIngredient.name
					].join(' ') + (chefkochIngredient.usageInfo || '');
				const ingredient = parseIngredient(ingredientString);
				ingredients.push(...ingredient);
			} catch (err) {
				throw new Error(
					'Unable to parse ingredient from chefoch recipe (ingredient string was "' +
						ingredientString +
						'"!'
				);
			}
	...
	const imageUrl = chefkochRecipe.previewImageUrlTemplate
		? chefkochRecipe.previewImageUrlTemplate
				.replaceAll('\\/', '/')
				.replace('<format>', 'crop-960x540')
		: undefined;
...
export const fetchRecipesFromAllUserCollections = onCall({ maxInstances: 1 }, async (request) => {
	...
	try {
		...
		const getUserFromReponse = async (response: Response) =>
			response.json().then((json) => json.id);
		...
		const getUserCollectionsUrl =
			'https://api.chefkoch.de/v2/cookbooks/user-' + userId + '/collections?offset=0&limit=100';
		const getUserCollectionsFromResponse = async (response: Response) =>
			response.json().then((json) => json.results.map((result: any) => result.id));
		...
		for (const collectionId of userCollections) {
			const getCollectionNameAndRecipesCountUrl =
				'https://api.chefkoch.de/v2/cookbooks/user-' +
				userId +
				'/recipes?categoryId=' +
				collectionId +
				'&offset=0&limit=0';
			const getCollectionNameAndRecipesCount = async (response: Response) =>
			...
			console.log(
				'=== Retrieving ' +
					collectionNameAndRecipesCount.count +
					' Recipes from Collection ' +
					collectionNameAndRecipesCount.name
			);

			const getCollectionRecipesUrl =
				'https://api.chefkoch.de/v2/cookbooks/user-' +
				userId +
				'/recipes?categoryId=' +
				collectionId +
				'&offset=0&limit=' +
				collectionNameAndRecipesCount.count;
			const getCollectionRecipes = async (response: Response) =>
		...
		throw new HttpsError(
			'internal',
			(error as Error)?.message ||
				'An error occurred while trying to request information from chefkoch.de!'
		);
	}
...
const fetchSingleRecipe = async (
	...
	return internalFetchRecipe(recipeId, chefkochCookie)
		.then((recipeDraft) => {
			return { ...recipeDraft, comment: recipe.note };
		})
		.catch((error) => {
			console.error(
				'An error occurred while trying to import recipe with ID ' +
					recipeId +
					' (' +
